...
class PokemonDataBox < SpriteWrapper
  ...
  def initializeOtherGraphics(viewport)
    ...
    @contents = BitmapWrapper.new(@databoxBitmap.width,@databoxBitmap.height+16)
  ...
  def refresh
    ...
    if @battler.opposes?(0) && defined?(@battler.type1) && defined?(@battler.type2)
      ...
      if type1_number == type2_number
        imagePos.push(["Graphics/Pictures/Battle/icon_types",@spriteBaseX+150,56,
                      0,(type1_number)*16,-1,16])
      else
        imagePos.push(["Graphics/Pictures/Battle/icon_types",@spriteBaseX+102+1,56,
                      0,(type1_number)*16,-1,16])
        imagePos.push(["Graphics/Pictures/Battle/icon_types",@spriteBaseX+150,56,
                      0,(type2_number)*16,-1,16])
      end
    end
