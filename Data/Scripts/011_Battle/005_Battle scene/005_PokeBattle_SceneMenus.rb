...
class FightMenuDisplay < BattleMenuBase
  ...
  def initialize(viewport,z)
    ...
    if USE_GRAPHICS
      ...
      # Create overlay for selected move's effectiveness
      @effectivenessOverlay = BitmapSprite.new(Graphics.width,Graphics.height-self.y,viewport)
      @effectivenessOverlay.x = self.x
      @effectivenessOverlay.y = self.y
      pbSetTinyFont(@effectivenessOverlay.bitmap)
      addSprite("effectivenessOverlay",@effectivenessOverlay)
      # Create type icon
  ...
  def z=(value)
    ...
    @effectivenessOverlay.z += 7 if @effectivenessOverlay
  ...
  def refreshSelection
    moves = (@battler) ? @battler.moves : []
    if USE_GRAPHICS
      @effectivenessOverlay.bitmap.clear
      textPos = []
      ...
      pbDrawTextPositions(@effectivenessOverlay.bitmap,textPos)
    end
